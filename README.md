...
- z=3.0 下界 = $-291，覆盖 ~0.5% 订单（教科书默认，太严）
- z=2.5 下界 = $-239，覆盖 ~2.5% 订单（与业务规则 `profit < -$200` 对齐）
- z=2.0 下界 = $-187，覆盖 ~4% 订单（消融验证 P/R/F1 均优于 2.5）
- 以上是**分布数学**（mean=$22, std=$104），不是检测结果。z ↓ → 理论覆盖订单 ↑，但实际检出几乎不变（业务规则和 IQR 已覆盖大部分）

> **接入新数据后如何重新校准阈值**：当前 z=2.0 是基于 DataCo 数据集消融实验选出的最优整数（原为 z=2.5，经校准下调）。接入公司真实数据后，运行 `python analysis/threshold_analysis.py --calibrate` 即可自动基于新数据分布输出推荐 z 值并写入 config.yaml。
...
全量识别:  61,678 条异常 (high=15,602, medium=32,109, low=11,874)
评测集:    106 条分层标注 (89 业务/边界/正常/模式样本 + 17 统计型异常)
```
...
> **消融实验核心发现**：Z-Score 和 IQR 在记录级独立贡献为 0，检测主力是业务规则；Ensemble 的增量主要来自日聚合移动平均层对订单量、延迟率、均值利润等趋势异常的捕获。当前阈值 **z=2.0**（整数，Precision/Recall/F1 均优于原 2.5），F1 最优在 z=1.4（75.2%），选 2.0 是精度与解释性的平衡。详见 `analysis/threshold_analysis.py`。
...
- [x] 评测体系（106 样本分层测试集 + 跨模型评委 V4 Pro + 3 项指标自动评测）
- [x] 归因质量优化（句式多样性 2→4、证据/逻辑两轮强化、V4 Flash 适配）
- [x] 测试体系（检测边界、归因 Agent、飞书卡片格式）
...
评测集 106 条分层抽样样本（89 业务/边界/正常/模式样本 + 17 统计型异常，见 `eval/README.md`）。旧评测集仅 10 条且缺负例，Precision 口径不可比——**以下为首次可信的性能基线**，后续优化均以此为基准。

#### 检测指标

| 指标 | 值 | 说明 |
|------|-----|------|
| Precision | 64.3% | 当前 z=2.0 |
| Recall | 83.3% | 当前 z=2.0 |
| F1 | **72.6%** | 当前 z=2.0（消融 Ensemble 最优 F1=75.2% at z=1.4） |
...
### 为什么改 z 值只影响 P/R/F1，不影响异常总数？

检测是四方法并集（Z-Score ∪ IQR ∪ 移动平均 ∪ 业务规则）。消融实验证明 Z-Score 记录级独立贡献为 0，主力是业务规则（单独 F1=64.2%）。调 z 值只改变 Ensemble 对边缘样本的判定边界（影响 P/R/F1），不影响总检出量（业务规则和 IQR 已覆盖大头）。
...
### 换模型
编辑 `config.yaml`，改两行即可切换到任何 OpenAI 兼容模型：
...
### 校准 z 值
```bash
# 自动分析当前数据 → 输出候选表 → 推荐最优 z → 自动写 config
python analysis/threshold_analysis.py --calibrate
# 或手动编辑 config.yaml → anomaly.zscore.threshold
```

### 改变归因数量
编辑 `skills/supply-chain-monitor/config.json`：
```json
{ "defaults": { "max_anomalies": 30 } }
```

### 换数据源
把新 CSV 放到 `data/raw/` 目录（列名需与 DataCo 一致，见「接入你自己的数据」）。然后：
```bash
python analysis/threshold_analysis.py --calibrate  # 先校准
python skills/supply-chain-monitor/monitor.py --mode daily  # 跑日报
```
