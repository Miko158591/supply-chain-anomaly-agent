...
评测集 89 条分层抽样样本（见 `eval/README.md`）。旧评测集仅 10 条且缺负例，Precision 口径不可比——**以下为首次可信的性能基线**，后续优化均以此为基准。

#### 检测指标

| 指标 | 值 | 说明 |
|------|-----|------|
| Precision | 55.4% | 含 30 条边界样本（占 34%）显著拉低；生产数据边界占比 ~10%，实际 Precision 预计更高 |
| Recall | **83.8%** | 极端异常基本全捕获，边界样本大部分被识别 |
| F1 | 66.7% | 评测集偏难（边界 34% vs 生产 ~10%），应视为**悲观下界** |
...
| 证据充分性 | 2.4 | 仍在改善中，evidence 需更多具体数据引用 |
