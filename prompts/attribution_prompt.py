...
2. **每条 evidence 必须包含至少一个具体数字或数据对比**（如"品类均值 $8.50 vs 全局 $26.39"），禁止仅引用 SOP 编号或笼统描述（如"该订单属于异常"）
3. **推理链要求**：每条 cause 必须从数据出发，经过对比基准→识别偏离→推断原因，不能跳过中间步骤直接给结论
4. **指标聚焦铁律**：shipping_delay_days→物流/仓储原因；Benefit per order→定价/折扣/运费成本；Order Item Profit Ratio→成本核算/定价策略。禁止在延迟异常中归因于利润结构，也禁止在利润异常中归因于物流延迟
4. 如果数据不足以确定某个原因，请如实标注 confidence 为低（< 0.5）并说明证据不足
5. 所有金额使用美元($)表示，时间使用 YYYY-MM-DD 格式
6. 回复必须是合法的 JSON，不包含任何 JSON 之外的文字

**evidence 质量铁律（违反任一条 = 不合格）**：
1. 每条 evidence 必须包含至少 1 个具体数字（金额/天数/百分比）
2. 每条 evidence 必须包含对比基准（vs 全局 / 品类均值 / 正常范围）
3. 禁止纯 SOP 引用作为 evidence——SOP 是通用规则，不是数据证据
4. **quality > quantity**：宁可 2 条带数字的强证据，不要 4 条笼统的弱证据
5. 多条 evidence 必须从不同维度论证（数值偏离 / 品类对比 / 状态特征），不能重复说同一件事

❌ 坏（纯 SOP）："SOP-002指出折扣叠加是亏损的两大主因"
❌ 坏（无数值）："该订单利润严重亏损"
❌ 坏（堆砌）：4 条全描述性事实（"订单$327" "段位Consumer"）— 无论证力
✅ 好："实际亏损 -$277.09，超出正常下界 -$239 约 16%"
✅ 好："品类近7天 $8.50 vs 全局 $26.39，差距 -$17.89（-68%）"

**表达多样性要求（非常重要）**：
你的输出会被供应链业务人员每天读到。模板化的表达会让人疲劳并降低对 AI 分析的信任度。
- 同一批次内的多条归因，句式必须各不相同。不要每条都用同一句式开头。
- 从以下表达模式中选择最贴切的一种，**每次轮换使用，同一批次内不重复**：
  模式 A（直接定位）："XXX 异常源于 YYY，具体表现为 ZZZ"
  模式 B（推断链路）："数据显示 XXX，结合 YYY 的历史模式，推断 ZZZ"
  模式 C（对比异常）："该 XXX 偏离同类平均 N%，与 YYY 因素强相关"
  模式 D（多假设并列）："有两个可能原因：1) XXX，2) YYY，证据更支持前者"
  模式 E（条件归因）："如果 XXX，则归因于 YYY；如果 ZZZ，则归因于 WWW"
- 用具体的业务术语，避免反复使用"主因是"、"叠加"、"排查"等通用词汇
- 适当使用领域词汇（"品类毛利率"、"承运商履约率"、"季节性波动"、"货位布局"、"出库扫描"等）
- 建议的句式也不应重复——有时可以建议"XX团队复核YY数据"，有时可以是"调整ZZ策略以减少WW风险"

---

## Few-Shot 示例（学习分析深度和证据质量，但不要复用这些示例的句式）
...
```json
{
  ...
  "summary": "订单#77202严重亏损源于延迟交付触发的运费补贴与客户补偿——该品类近7天利润均值仅$8.50（全局$26.39），利润空间极薄，任何额外成本都会将其推入亏损。"
}
```
...
```json
{
  "root_cause_hypotheses": [
    {
      "cause": "品类近7天平均延迟0.68天，比全局均值0.46天高出48%，提示该品类的供应链可能存在系统性问题。结合鞋类SKU多（尺码×颜色×宽度）、拣货复杂度高的特点，最可能的瓶颈在仓库出库环节——拣货效率不足导致该品类订单普遍延迟。",
      "probability": 0.70,
      "evidence": [
  ...
  "summary": "数据显示品类近7天延迟均值0.68天（全局0.46天），结合该订单实际延迟4天远超品类均值，推断瓶颈在仓库拣货效率——建议排查该品类货位布局和订单出库扫描记录。"
}
```
...
```json
{
  "root_cause_hypotheses": [
    {
      "cause": "有两个可能原因：1) 采购成本数据未及时更新，导致系统中成本基准偏低、利润率虚高——品类近7天均值仅0.08而该订单0.47，差距近5倍；2) SKU售价在促销结束后未恢复原价。当前证据更支持第一种解释，因为品类均值0.08远低于全局0.12，说明整个品类的成本核算可能存在问题。",
      "probability": 0.65,
      "evidence": [
  ...
  "summary": "该订单利润率0.47是品类均值的近6倍——如果成本数据准确，则需排查定价系统；如果成本缺失，则需批量修正该SKU的成本基准。两个方向都指向数据校验。"
}
```
...
- **不要复用上述示例的句式、措辞、或表达结构**——每次根据数据特征选择最自然的表达方式
- 同一批次分析多条异常时，每条 summary 和 cause 的句式必须各不相同
...
**质量标准（评分依据）**：
- root_cause_hypotheses 必须给出恰好 3 个假设（不足 3 个时 probability 设为 0 并说明证据不足）
- recommended_actions 必须给出恰好 3 条措施
- 所有数值必须来源于上下文数据，不得编造
- **证据充分性（关键）**：每条 evidence 至少包含 1 个具体数值 + 1 个对比基准（如"品类 X vs 全局 Y，偏离 Z%")，禁止纯 SOP 引用或笼统描述
- **逻辑连贯性（关键）**：每条 cause 需展示推理链——数据观察 → 对比偏离 → 推断机制 → 根因结论，不能跳跃
- 如果某个假设缺乏证据，probability 应 < 0.3，并在 against 中诚实说明
- **表达多样性**：summary 和 cause 不要使用"主因是 X 叠加 Y"模板，从表达模式 A~E 中轮换；3 条假设的 cause 和 3 条建议的 action 各有不同句式
- **逻辑自检（最重要）**：第一步先确认异常指标类型——metric=shipping_delay_days 的异常原因必然是物流/仓储/承运商问题，metric=Benefit per order 或 Order Item Profit Ratio 的异常原因才是定价/折扣/成本问题。**禁止混淆指标类型**：不要在延迟异常的 cause 中分析利润亏损，也不要在利润异常的 cause 中分析物流延迟。
