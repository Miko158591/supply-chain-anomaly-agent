...
| 指标 | 值 | 说明 |
|------|-----|------|
| 检测 F1 | 67.2% | z=2.5 基线（消融最优可达 75.2%） |
| Precision | 61.5% | 含 17 条统计型异常，Recall 优先设计 |
| Recall | 74.1% | 17 条统计型样本未能检出（记录级盲区） |
| 归因整体 | 3.4/5 | V4 Pro 评委（跨版本） |
| 行动可操作性 | 5.0/5 | 建议具体有负责人 |
| 证据充分性 | 2.2/5 | prompt 天花板，瓶颈在数据粒度 |
| ROI | $156,139/年 | What-if 估算（保守 30%/20%/15% 挽回率） |
...
2. **z=2.5 数据驱动**（ADR-002）：基于 DataCo 实际分位数，支持 `--calibrate` 自动校准
