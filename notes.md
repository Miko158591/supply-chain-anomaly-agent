...
### 1. System prompt 证据质量铁律
- 5 条硬性规则（每条 evidence 必须有数字+对比基准+禁止 SOP-only+quality>quantity+多维度论证）
- 新增坏 evidence 反例（纯 SOP/无数值/堆砌事实/重复论证）
- 新增好 evidence 正例（数值+偏离度/数值+对比）

### 2. 好坏示例文件
- `prompts/good_evidence.json`: 3 个高质量 evidence 示例（利润/延迟/利润率，每条含 type+content）
- `prompts/bad_evidence.json`: 4 个反模式（笼统/SOP/堆砌/重复）

### 3. 评测结果

| 案例 | 证据分 | 评语 |
|------|--------|------|
| TC001 | 2/5 | 证据合理但缺乏对比基准，数据基础偏弱 |
| TC002 | 2/5 | 有量化但缺乏对比基准 |
| TC003 | 2/5 | 证据缺品类基准和数值对比 |
| TC004 | 2/5 | 缺少数值对比基准，依赖 SOP 推理 |
| TC005 | 3/5 | 证据部分未使用数值对比基准 |

## 关键发现：数据瓶颈

**prompt 规则只能优化证据的表达形式，无法创造不存在的数据。** 当前 context 生成仅提供：
- 品类级对比（品类均值 vs 全局均值）
- 订单基本字段（利润/延迟/状态）
...
**结论**：证据分 2.0→2.2 已达 prompt 优化天花板。要突破 3.5+，需接入更丰富的运营数据源（WMS/TMS/ERP）提供订单级细粒度对比基准。这属于数据工程范畴，超出当前 DataCo 数据集的能力边界。
