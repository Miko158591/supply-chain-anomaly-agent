...
从 DataCo Smart Supply Chain Dataset（180,519 行）中按业务异常、边界样本、正常样本和统计型异常分层抽样：
...
| E: 统计型异常 | 17 | 日订单量飙升 / 单笔金额超 P95 / 品类集中下单 | 验证日聚合与统计层补充能力 |

**总计**: 106 条（89 条业务/边界/正常/模式样本 + 17 条统计型异常补充样本）
...
| 指标 | 值 | 说明 |
|------|-----|------|
| Precision | 64.3% | z=2.0 当前基线 |
| Recall | 83.3% | z=2.0 当前基线 |
| F1 | 72.6% | z=2.0 当前基线 |
...
**为什么保留边界样本**：评测集中包含利润 -\$200~\$0、延迟 2-4 天等模糊区间，用来模拟真实业务中"值得关注但不一定必须报警"的灰区。这会让指标更保守，但比只测极端异常更接近生产环境。

**为什么 Recall 优先**：供应链异常监控场景下漏报的代价更高——延迟+亏损订单没被发现，可能继续影响客户体验、履约和财务损失；误报一条通常只是多一次人工复核。业务规则层负责兜底高风险异常，统计/日聚合层负责补充趋势异常。
